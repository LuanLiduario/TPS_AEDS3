...
void lerArquivo(char *nomeArq,char *nomeSaida){
    ...
    while(!feof(arq)){//percorre o arquivo
        ...
        i++;
        if(c==' ' || c=='\n' || feof(arq)){//caso o caracter recebido encontrar espaço ou \n ou o fim do arquivo significa que a acabou a leitura de um número de vértice
            linha[i] = '\0';
    ...
    aux=guloso(vetorArestas,v,grafo);//chama a função guloso para calcular a solução
